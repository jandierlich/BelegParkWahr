...
- **Eigenständige Startseite** (`index.html`): Einstiegspunkt mit App-Icon, kurzer Beschreibung und Links zu App, Anleitung, Über die App, Impressum und Datenschutz — bitte deine Kontaktdaten in `impressum.html` und `datenschutz.html` eintragen (Platzhalter in eckigen Klammern)
...
2. Klicke auf **"New repository"**, gib z. B. `parkenwahr` als Namen ein, wähle **Public**, dann **"Create repository"**
3. Klicke auf **"uploading an existing file"** (oder "Add file" → "Upload files")
4. Ziehe **alle 13 Dateien aus diesem Ordner flach (kein Unterordner)** hinein — index.html, app.html, style.css, app.js, db.js, crop.js, voice.js, app.webmanifest, sw.js, impressum.html, datenschutz.html, icon-192.png, icon-512.png. Es reicht, die Dateien auszuwählen/hineinzuziehen; Inhalte müssen **nicht** manuell kopiert werden. Bestätige mit "Commit changes"
...
7. Nach 1–2 Minuten ist die App erreichbar unter:
   `https://DEIN-BENUTZERNAME.github.io/parkenwahr/`
...
1. Link in **Safari** öffnen (wichtig: Safari, nicht Chrome — nur Safari kann PWAs auf iOS installieren) — du landest zuerst auf der Startseite
2. Auf **"App öffnen"** tippen, dann in Safari das Teilen-Symbol antippen → **"Zum Home-Bildschirm"**
3. Die App erscheint als eigenes Icon und öffnet sich direkt (ohne Umweg über die Startseite), ohne Browser-Leiste
