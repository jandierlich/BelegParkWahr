// app.js — Hauptlogik der Web-App "BelegParkWahr"
...
function celebrateConfirm() {
  const colors = ['#3F53F2', '#FF4433', '#00BD90', '#FF8C00', '#E8267A'];
...
async function loadAll() {
  ...
  // App startet bewusst immer im Hellmodus, unabhängig von einem zuvor gespeicherten
  // Zustand oder der iPhone-Systemeinstellung. Der Umschalter bleibt für die aktuelle
  // Sitzung nutzbar, wirkt sich aber nicht mehr auf den nächsten App-Start aus.
  state.darkMode = false;
  document.body.classList.remove('dark');
  await dbSetMeta('darkMode', false);
  const themeMeta = document.querySelector('meta[name="theme-color"]');
  if (themeMeta) themeMeta.setAttribute('content', '#0B4F49');
}
...
function header(sub, title, opts = {}) {
  ...
  // Einheitlicher Home-Button auf jeder einzelnen Seite: führt immer zur echten
  // Startseite (index.html) — unabhängig davon, was rightContent sonst noch enthält.
  const homeBtn = el('a', { href: 'index.html', class: 'header-btn', style: 'text-decoration:none;display:flex', html: homeSvg() });
  const h = el('div', { class: opts.variant ? `header header-${opts.variant}` : 'header' }, [
    ...
    el('div', { class: 'header-row' }, [
      ...
      el('div', { style: 'display:flex;gap:8px' }, [homeBtn, rightContent]),
    ]),
  ]);
  return h;
}
...
const chevronRightSvg = () => `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 6 15 12 9 18"/></svg>`;
...
function renderDashboard() {
  ...
  body.appendChild(el('div', { style: 'display:flex;align-items:center;gap:8px;margin:16px 0 14px' }, [
    el('span', { style: 'font-size:22px' }, greeting.emoji),
    el('span', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;color:var(--ink)' }, greeting.text),
  ]));
  ...
  body.appendChild(el('div', { class: 'stat-strip' }, [
    el('div', { class: 'card stat-tile' }, [
      el('div', { style: 'font-size:10px;color:var(--sub);font-family:ui-monospace, monospace;letter-spacing:1px' }, 'PARKSCHEINE'),
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;margin-top:4px' }, `${parkMonth.length}`),
      el('div', { style: 'font-family:ui-monospace, monospace;font-size:13px;color:var(--amount);font-weight:700' }, fmtEUR(parkTotal)),
    ]),
    el('div', { class: 'card stat-tile' }, [
      el('div', { style: 'font-size:10px;color:var(--sub);font-family:ui-monospace, monospace;letter-spacing:1px' }, 'NEBENKOSTEN'),
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;margin-top:4px' }, `${extraMonth.length}`),
      el('div', { style: 'font-family:ui-monospace, monospace;font-size:13px;color:var(--amount);font-weight:700' }, fmtEUR(extraTotal)),
    ]),
    el('div', { class: 'card stat-tile' }, [
      el('div', { style: 'font-size:10px;color:var(--sub);font-family:ui-monospace, monospace;letter-spacing:1px' }, 'FAHRTEN'),
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;margin-top:4px' }, `${fahrtKm.toLocaleString('de-DE')} km`),
      el('div', { style: 'font-family:ui-monospace, monospace;font-size:13px;color:var(--amount);font-weight:700' }, fmtEUR(fahrtTotal)),
    ]),
  ]));
...
function renderHome() {
  ...
  wrap.appendChild(header('BelegParkWahr', monthNames[new Date().getMonth()] + ' ' + new Date().getFullYear(), {
    right: el('button', { class: 'header-btn', onclick: () => { state.screen = 'search'; render(); }, html: searchSvg() }),
  }));
...
function showUndoToast(store, entry) {
  ...
  const toast = el('div', {
    ...
    el('span', { style: 'font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px' }, 'Eintrag gelöscht'),
    el('button', {
      style: 'background:var(--violet);color:#fff;border:none;border-radius:16px;padding:8px 14px;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;cursor:pointer;flex-shrink:0;',
      onclick: async () => {
...
function renderVoiceRecorder() {
  ...
  if (state.capturedAudio) {
    const row = el('div', { style: 'display:flex;align-items:center;gap:10px;background:var(--card);border:2px solid var(--teal);border-radius:20px;padding:10px 14px' }, [
      ...
      el('span', { style: 'flex:1;font-size:13px;color:var(--ink);font-family:-apple-system, BlinkMacSystemFont, sans-serif' }, 'Sprachnotiz aufgenommen'),
  ...
  if (state.isRecording) {
    const btn = el('button', {
      style: 'width:100%;padding:13px;border-radius:20px;border:none;background:var(--coral);color:#fff;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer',
      onclick: async () => {
  ...
  const btn = el('button', {
    style: 'width:100%;padding:13px;border-radius:20px;border:2px dashed var(--sub);background:transparent;color:var(--ink);font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer',
    onclick: async () => {
      try {
...
function ocrField(label, stateKey, confirmedKey, type, color) {
  ...
  const inputEl = el('input', {
    ...
    style: 'border:none;background:transparent;margin:0;padding:0;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;color:var(--ink);width:100%;',
  });
...
function renderExtra() {
  ...
  ['Alle', ...state.categories].forEach(cat => {
    ...
    const row = el('div', { style: 'display:flex;align-items:stretch;gap:6px' }, [
      el('button', {
        style: `flex:1;text-align:left;padding:12px 16px;border-radius:20px;border:none;cursor:pointer;font-family:ui-monospace, monospace;font-size:13px;font-weight:600;background:${active ? 'var(--violet)' : 'rgba(108,76,224,0.1)'};color:${active ? '#fff' : 'var(--violet)'}`,
  ...
  chipList.appendChild(el('button', {
    style: 'width:100%;text-align:left;padding:12px 16px;border-radius:20px;border:2px dashed var(--sub);background:transparent;color:var(--sub);font-family:ui-monospace, monospace;font-size:13px;font-weight:600;display:flex;align-items:center;gap:6px;cursor:pointer',
...
const catColors = { Betriebsmittel: '#FF8C00', Spesen: '#00BD90', Hotelrechnung: '#E8267A' };
const catColorsDark = { Betriebsmittel: '#FFA020', Spesen: '#17E6B5', Hotelrechnung: '#FF5CA8' };
...
function renderAddExtra() {
  ...
  if (!existing || !existing.recurringSourceId) {
    ...
    recurringRow.appendChild(el('div', {}, [
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:13px' }, '🔁 Wiederkehrend'),
...
function renderFahrtenContent() {
  ...
  frag.appendChild(el('div', { class: 'card', style: 'display:flex;justify-content:space-between;align-items:center' }, [
    el('div', {}, [
      ...
      el('div', { style: 'font-family:ui-monospace, monospace;font-size:11px;color:var(--violet);font-weight:600;margin-top:2px' }, `${totalKm.toFixed(1)} km`),
    ]),
    el('span', { style: 'font-family:ui-monospace, monospace;font-size:17px;font-weight:700;color:var(--amount)' }, fmtEUR(totalBetrag)),
  ]));
...
function renderSearchScreen() {
  ...
  function runSearch(q) {
    ...
    function section(title, count) {
      if (count === 0) return;
      resultsWrap.appendChild(el('div', { style: 'font-family:ui-monospace, monospace;font-size:10px;letter-spacing:1px;color:var(--sub);margin:16px 0 8px' }, `${title.toUpperCase()} (${count})`));
    }
...
function renderGuideScreen() {
  ...
  const sections = [
    ...
    },
    {
      ...
      icon: briefcaseSvg, color: '#00BD90',
      steps: [
  ...
  sections.forEach(sec => {
    body.appendChild(el('div', { class: 'card' }, [
      el('div', { style: 'display:flex;align-items:center;gap:10px;margin-bottom:10px' }, [
        ...
        el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px' }, sec.title),
      ]),
      ...sec.steps.map((s, i) => el('div', { style: 'display:flex;gap:8px;margin-bottom:6px' }, [
        el('span', { style: 'font-family:ui-monospace, monospace;font-size:12px;color:var(--sub);flex-shrink:0' }, `${i + 1}.`),
        el('span', { style: 'font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;color:var(--sub);line-height:1.4' }, s),
      ])),
    ]));
  });

  body.appendChild(el('div', { style: 'text-align:center;font-size:11px;color:var(--sub);margin-top:8px;font-family:ui-monospace, monospace' },
...
function renderAboutScreen() {
  ...
  body.appendChild(el('div', { style: 'font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;color:var(--sub);margin-bottom:16px;text-align:center' },
  ...
  const items = [
    ...
    { icon: briefcaseSvg, color: isDark ? '#17E6B5' : '#00BD90', title: 'Nebenkosten', text: 'Mit frei anpassbaren Kategorien (umbenennbar, löschbar), Notizen und Belegfoto.' },
    ...
    { icon: mapSvg, color: isDark ? '#17E6B5' : '#00BD90', title: 'Karte', text: 'Zeigt jeden erfassten Parkort einzeln mit Standort an.' },
  ...
  items.forEach(it => {
    body.appendChild(el('div', { class: 'card', style: 'display:flex;gap:12px;align-items:flex-start' }, [
      ...
      el('div', {}, [
        el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;margin-bottom:2px' }, it.title),
        el('div', { style: 'font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;color:var(--sub);line-height:1.4' }, it.text),
      ]),
    ]));
  });
...
function renderMap() {
  ...
  if (lastPark) {
    ...
    infoCard.appendChild(el('div', { class: 'card', style: 'margin:0;border-left:4px solid var(--violet)' }, [
      el('div', { style: 'font-size:10px;color:var(--violet);font-family:ui-monospace, monospace;letter-spacing:0.5px;margin-bottom:2px' },
        allParkWithLocation.length > 1 ? `${allParkWithLocation.length} PARKORTE AUF DER KARTE` : 'ZULETZT GEPARKT'),
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px' }, lastPark.ort || 'Parkort'),
      el('div', { style: 'font-family:ui-monospace, monospace;font-size:11px;color:var(--sub);margin:4px 0 12px' }, new Date(lastPark.date).toLocaleDateString('de-DE') + (allParkWithLocation.length > 1 ? ' (neuester)' : '')),
      el('a', {
        href: mapsUrl,
        style: 'display:flex;align-items:center;justify-content:center;gap:8px;padding:11px;border-radius:20px;background:linear-gradient(135deg,var(--violet),var(--violet-deep));color:#fff;text-decoration:none;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px',
  ...
  if (lastKm) {
    ...
    infoCard.appendChild(el('div', { class: 'card', style: 'margin:0;border-left:4px solid var(--teal)' }, [
      el('div', { style: 'font-size:10px;color:var(--teal);font-family:ui-monospace, monospace;letter-spacing:0.5px;margin-bottom:2px' }, 'LETZTER KILOMETERSTAND'),
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px' }, `${lastKm.km.toLocaleString('de-DE')} km${lastKm.ort ? ' · ' + lastKm.ort : ''}`),
      el('div', { style: 'font-family:ui-monospace, monospace;font-size:11px;color:var(--sub);margin:4px 0 12px' }, new Date(lastKm.date).toLocaleDateString('de-DE')),
      el('a', {
        href: mapsUrl,
        style: 'display:flex;align-items:center;justify-content:center;gap:8px;padding:11px;border-radius:20px;background:linear-gradient(135deg,var(--teal),#00A183);color:#fff;text-decoration:none;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px',
      }, [icon(mapPinSvg(), 15), ' Standort auf OpenStreetMap öffnen']),
    ]));
  }
  ...
  const mapWrap = el('div', { class: 'map-wrap' }, [
    el('div', { id: 'map' }),
    el('div', { id: 'map-status', style: 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;text-align:center;padding:20px;font-size:13px;color:var(--sub);font-family:-apple-system, BlinkMacSystemFont, sans-serif;background:var(--bg);' }, 'Karte wird geladen …'),
  ]);
...
function initMap() {
  ...
  if (lastKm) {
    ...
    const kmIcon = L.divIcon({ html: `<div style="background:#00BD90;width:26px;height:26px;border-radius:50%;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.3)"></div>`, className: '', iconSize: [26, 26], iconAnchor: [13, 13] });
...
function renderMonth() {
  ...
  if (state.overviewMode === 'year') {
    ...
    const monthNav = el('div', { style: 'display:flex;align-items:center;justify-content:space-between;margin-bottom:14px' }, [
      ...
      el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px' }, monthLabel(selectedMonthKey())),
      el('button', {
  ...
  const fullBackupCard = el('div', { class: 'card', style: 'margin-top:8px' }, [
    el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;margin-bottom:4px' }, '🗄️ Vollständiges Backup'),
  ...
  const xlsxCard = el('div', { class: 'card', style: 'margin-top:8px' }, [
    el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;margin-bottom:4px' }, '📊 Excel-Export'),
  ...
  [
    ...
    xlsxCatRow.appendChild(el('button', {
      style: `padding:8px 12px;border-radius:28px;border:none;cursor:pointer;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-size:12px;font-weight:700;background:${active ? 'var(--violet)' : 'rgba(108,76,224,0.1)'};color:${active ? '#fff' : 'var(--violet)'}`,
...
function pdfCheckRow(label, count, unit, total, key, color) {
  ...
  return el('div', {
    ...
    el('span', { style: 'display:flex;align-items:center;gap:10px' }, [
      ...
      el('span', {}, [label + ' ', el('span', { style: 'font-family:ui-monospace, monospace;font-size:10px;color:var(--sub)' }, `(${count} ${unit})`)]),
    ]),
    el('span', { style: 'font-family:ui-monospace, monospace;font-weight:700;font-size:17px;color:var(--amount)' }, fmtEUR(total)),
  ]);
}
...
function renderMonthContent() {
  ...
  frag.appendChild(el('div', { class: 'card' }, [
    el('div', { style: 'font-size:10px;color:var(--sub);font-family:ui-monospace, monospace;letter-spacing:1px' }, 'AUSGABENVERLAUF (6 MONATE BIS AUSGEWÄHLTEM MONAT)'),
    el('div', { class: 'chart-bars' }, trend.map(v => el('div', { style: `height:${Math.max((v / max) * 100, 4)}%` }))),
  ]));

  frag.appendChild(el('div', { style: 'font-size:10px;color:var(--sub);font-family:ui-monospace, monospace;letter-spacing:1px;margin:4px 0 8px' }, 'FÜR PDF/CSV AUSWÄHLEN (ANTIPPEN)'));
  ...
  frag.appendChild(el('div', { class: 'total-strip' }, [
    ...
    el('span', { style: 'font-family:ui-monospace, monospace;font-weight:700;font-size:17px' }, fmtEUR(selectedTotal)),
  ]));
...
function renderYearContent() {
  ...
  frag.appendChild(el('div', { class: 'card' }, [
    el('div', { style: 'font-size:10px;color:var(--sub);font-family:ui-monospace, monospace;letter-spacing:1px' }, `MONATSVERLAUF ${year}`),
    ...
    el('div', { style: 'display:flex;justify-content:space-between;margin-top:4px' },
      ['J','F','M','A','M','J','J','A','S','O','N','D'].map(l => el('span', { style: 'font-size:9px;color:var(--sub);flex:1;text-align:center;font-family:ui-monospace, monospace' }, l))),
  ]));
...
async function buildXLSX() {
  if (typeof XLSX === 'undefined') { await customAlert('Excel-Bibliothek konnte nicht geladen werden.'); throw new Error('xlsx missing'); }
  const sel = state.xlsxSelection;
...
function showXLSXPreview(filtered) {
  ...
  title.style.cssText = "color:#fff;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;";
  const closeBtn = document.createElement('button');
  closeBtn.textContent = 'Schließen ✕';
  closeBtn.style.cssText = "background:rgba(255,255,255,0.15);color:#fff;border:none;border-radius:16px;padding:8px 14px;font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;";
  ...
  function section(name, headers, rows) {
    ...
    h.style.cssText = "font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;color:#2E3448;margin-bottom:6px;";
    wrapDiv.appendChild(h);
    if (rows.length === 0) {
      const empty = document.createElement('div');
      empty.textContent = 'Keine Einträge in diesem Zeitraum.';
      empty.style.cssText = "font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;color:#7C88A3;";
      wrapDiv.appendChild(empty);
    } else {
      const table = document.createElement('table');
      table.style.cssText = 'width:100%;border-collapse:collapse;font-family:ui-monospace, monospace;font-size:11px;color:#2E3448;';
...
function showFilePreview(blob, mimeType) {
  ...
  if (mimeType === 'application/pdf') {
    ...
    box.innerHTML = `<div style="font-family:-apple-system, BlinkMacSystemFont, sans-serif;color:#2E3448;font-size:14px">Für diesen Dateityp gibt es keine Vorschau hier in der App.<br><br>Die Datei ist aber fertig und bereit zum Speichern.</div>`;
    overlay.appendChild(box);
  } else {
    ...
    pre.style.cssText = "margin:0;white-space:pre-wrap;word-break:break-word;font-family:ui-monospace, monospace;font-size:13px;color:#2E3448;";
...
function toggleDarkMode() {
  ...
  const themeMeta = document.querySelector('meta[name="theme-color"]');
  if (themeMeta) themeMeta.setAttribute('content', state.darkMode ? '#122B2E' : '#0B4F49');
  render();
}
...
(async function init() {
  await loadAll();
  resetAddForm();
  // Erlaubt Direktsprünge von der Startseite (index.html) aus, z. B. app.html?screen=guide
